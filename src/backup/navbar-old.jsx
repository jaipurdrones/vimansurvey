...
export default function Navbar() {
  ...
  const [activeDropdown, setActiveDropdown] = React.useState(null); // 🟢 track which dropdown is open
  const location = useLocation();

  // 🟢 Scroll to top and close all menus when route changes
  React.useEffect(() => {
    window.scrollTo({ top: 0, behavior: "smooth" });
    setMobileOpen(false);
    setActiveDropdown(null); // close dropdowns too
  }, [location.pathname]);
    ...
    <header className="fixed top-0 left-0 w-full z-50 bg-primary-200 shadow-sm border-b border-gray-100">
      <div className="max-w-7xl mx-auto flex justify-between items-center px-6 py-3">
        ...
        <Link to="/" className="flex items-center gap-3">
          <img src={logo} alt="Viman Survey Logo" className="h-20 object-contain" />
        </Link>
